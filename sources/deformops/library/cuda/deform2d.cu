...
template <typename scalar_t, int64_t d_stride, typename transfer_t, int64_t L,
          int64_t K, bool softmax>
__global__ void backward_kernel(
    const scalar_t *p_value, const scalar_t *p_offset,
    const scalar_t *grad_output, const int64_t G, const int64_t D,
    const int64_t Q, const int64_t kernel_h, const int64_t kernel_w,
    ...
    const int64_t height_in, const int64_t width_in, const int64_t height_out,
    const int64_t width_out, const opmath_t offset_scale,
    const bool remove_center, const int64_t block_multiplier, opmath_t *grad_im,
    opmath_t *grad_offset, const int64_t padded_offset_dim) {
  ...
  if (softmax) {
    ...
    if (threadIdx.x == 0) { // gi != 0, di = 0, li = 0
      ...
      // Normalize according to the sum
      for (int j = 0; j < K; j++) {
  ...
  const int64_t p0_w = ((dilation_w * (kernel_w - 1)) >> 1) - pad_w +
                       (qi % width_out) * stride_w;
  const int64_t p0_h = ((dilation_h * (kernel_h - 1)) >> 1) - pad_h +
  ...
  const int64_t center_h = kernel_h / 2;
  const int64_t center_w = kernel_w / 2;
...
__global__ void backward_kernel_warp_primitive(
  ...
  // Find current group position w.r.t. warp and lane
...
// Col2Im
template <typename scalar_t, typename stride_type, int64_t d_stride>
void col2im_strided(cudaStream_t stream,
                    ...
                    const int64_t G, const int64_t D, const int64_t B,
                    const int64_t height_in, const int64_t width_in,
                    const int64_t height_out, const int64_t width_out,
                    const opmath_t offset_scale, const bool remove_center,
                    opmath_t *grad_im, opmath_t *grad_offset,
                    const int64_t block_thread, const bool softmax,
                    const int64_t padded_offset_dim) {
  auto kernel = backward_kernel_warp_primitive<scalar_t, d_stride, stride_type,
                                               1, 9, false>;

  int64_t Q = height_out * width_out;
  int64_t K = kernel_h * kernel_w;
  ...
  if (softmax) {
    ...
    case 9:
      if (check_backward_warp(d_stride, D)) {
        ...
        kernel = backward_kernel<scalar_t, d_stride, stride_type, 1, 9, true>;
      }
      break;
    case 8:
      if (check_backward_warp(d_stride, D)) {
        ...
        kernel = backward_kernel<scalar_t, d_stride, stride_type, 1, 8, true>;
      }
      break;
    default:
      printf("K=%ld\n", K);
    ...
    case 9:
      if (check_backward_warp(d_stride, D)) {
        ...
        kernel = backward_kernel<scalar_t, d_stride, stride_type, 1, 9, false>;
      }
      break;
    case 8:
      if (check_backward_warp(d_stride, D)) {
        ...
        kernel = backward_kernel<scalar_t, d_stride, stride_type, 1, 8, false>;
      }
      break;
    default:
      printf("K=%ld\n", K);
  ...
  const int64_t blockdimX = D / d_stride;

  int64_t shm_size = sizeof(opmath_t) * (G * block_multiplier * K) * 2;
  if (!check_backward_warp(d_stride, D)) {
...
void col2im_cuda(cudaStream_t stream,
                 ...
                 const bool softmax, const int64_t padded_offset_dim) {
  assert(D % d_stride == 0);
  const int64_t size_scalar = sizeof(scalar_t);
  if (size_scalar == 2) {
    switch (d_stride) {
    case 1:
      col2im_strided<scalar_t, scalar_t, 1>(
      ...
      break;
    case 2:
      col2im_strided<scalar_t, uint, 2>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
      break;
    case 4:
      col2im_strided<scalar_t, uint2, 4>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
      break;
    case 8:
      col2im_strided<scalar_t, uint4, 8>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
      break;
    case 16:
      col2im_strided<scalar_t, ulonglong4, 16>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
    ...
    switch (d_stride) {
    case 1:
      col2im_strided<scalar_t, uint, 1>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
      break;
    case 2:
      col2im_strided<scalar_t, uint2, 2>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
      break;
    case 4:
      col2im_strided<scalar_t, uint4, 4>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
      break;
    case 8:
      col2im_strided<scalar_t, ulonglong4, 8>(
          stream, value, p_offset, grad_output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center, grad_im,
          grad_offset, block_thread, softmax, padded_offset_dim);
...
__global__ void forward_kernel(
    ...
    const int64_t G, const int64_t D, const int64_t Q, const int64_t kernel_h,
    ...
    const int64_t dilation_w, const int64_t height_in, const int64_t width_in,
    const int64_t height_out, const int64_t width_out,
    const opmath_t offset_scale, const bool remove_center,
    const int64_t block_multiplier, const int64_t padded_offset_dim) {
  const int64_t &qi = (blockIdx.x * block_multiplier % Q) + threadIdx.z;
  const int64_t &bi = blockIdx.x * block_multiplier / Q;
  const int64_t &di_s = threadIdx.x * d_stride;
  const int64_t &gi = threadIdx.y;
...
// Template function for im2col_cuda
template <typename scalar_t, typename stride_type, int64_t d_stride>
void im2col_strided(cudaStream_t stream,
                    const scalar_t *value,    // B, H * W, (G * D)
                    const scalar_t *p_offset, // B, H * W, G * K * 3)
                    ...
                    const int64_t height_out, const int64_t width_out,
                    const opmath_t offset_scale, const bool remove_center,
                    const int64_t block_thread, const int64_t softmax,
                    const int64_t padded_offset_dim) {
  auto kernel = forward_kernel<scalar_t, d_stride, stride_type, 1, 9, true>;
  ...
  if (softmax) {
    switch (K) {
    case 9:
      kernel = forward_kernel<scalar_t, d_stride, stride_type, 1, 9, true>;
      break;
    case 8:
      kernel = forward_kernel<scalar_t, d_stride, stride_type, 1, 8, true>;
      break;
    default:
      printf("K=%ld\n", K);
      throw std::invalid_argument("invalid kernel shape");
    ...
    switch (K) {
    case 9:
      kernel = forward_kernel<scalar_t, d_stride, stride_type, 1, 9, false>;
      break;
    case 8:
      kernel = forward_kernel<scalar_t, d_stride, stride_type, 1, 8, false>;
      break;
    default:
      printf("K=%ld\n", K);
      throw std::invalid_argument("invalid kernel shape");
...
void im2col_cuda(cudaStream_t stream,
  ...
  if (sizeof(scalar_t) == 2) {
    ...
    switch (d_stride) {
    case 1:
      im2col_strided<scalar_t, scalar_t, 1>(
      ...
      break;
    case 2:
      im2col_strided<scalar_t, uint, 2>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    case 4:
      im2col_strided<scalar_t, uint2, 4>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    case 8:
      im2col_strided<scalar_t, uint4, 8>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    case 16:
      im2col_strided<scalar_t, ulonglong4, 16>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
    ...
    switch (d_stride) {
    case 1:
      im2col_strided<scalar_t, uint, 1>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    case 2:
      im2col_strided<scalar_t, uint2, 2>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    case 4:
      im2col_strided<scalar_t, uint4, 4>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    case 8:
      im2col_strided<scalar_t, ulonglong4, 8>(
          stream, value, p_offset, output, kernel_h, kernel_w, stride_h,
          stride_w, pad_h, pad_w, dilation_h, dilation_w, G, D, B, height_in,
          width_in, height_out, width_out, offset_scale, remove_center,
          block_thread, softmax, padded_offset_dim);
      break;
    default:
      printf("not supported: d_stride > 8 for fp32");
      throw std::invalid_argument("invalid d_stride");
    }
  }
}

// Forward implementation on CUDA
at::Tensor forward_cuda(const at::Tensor &value, const at::Tensor &p_offset,
                        const int64_t kernel_h, const int64_t kernel_w,
                        const int64_t stride_h, const int64_t stride_w,
                        const int64_t pad_h, const int64_t pad_w,
                        const int64_t dilation_h, const int64_t dilation_w,
                        const int64_t group, const int64_t group_channels,
                        const double offset_scale, const int64_t im2col_step,
                        const bool remove_center, const int64_t d_stride,
                        const int64_t block_thread, const bool softmax) {
  CHECK_INPUT(value);
  CHECK_INPUT(p_offset);
  ...
  const int64_t padded_offset_dim = p_offset.size(3);

  CHECK_TENSORCORE(padded_offset_dim);
  ...
  CHECK_EQUALS(channels, group * group_channels);
  ...
  for (int n = 0; n < batch / im2col_step_; ++n) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        "deform2d::forward_cuda", ([&] {
          im2col_cuda(at::cuda::getCurrentCUDAStream(),
                      value.data_ptr<scalar_t>() +
                          n * im2col_step_ * per_value_size,
                      p_offset.data_ptr<scalar_t>() +
                          n * im2col_step_ * per_offset_size,
                      columns.data_ptr<scalar_t>(), kernel_h, kernel_w,
                      stride_h, stride_w, pad_h, pad_w, dilation_h, dilation_w,
                      group, group_channels, batch_n, height_in, width_in,
...
// Backward implementation on CUDA
std::vector<at::Tensor> backward_cuda(
    const at::Tensor &value, const at::Tensor &p_offset, const int64_t kernel_h,
    const int64_t kernel_w, const int64_t stride_h, const int64_t stride_w,
    const int64_t pad_h, const int64_t pad_w, const int64_t dilation_h,
    const int64_t dilation_w, const int64_t group, const int64_t group_channels,
    const double offset_scale, const int64_t im2col_step,
    const at::Tensor &grad_output, const bool remove_center,
    const int64_t d_stride, const int64_t block_thread, const bool softmax) {
  CHECK_INPUT(value)
  CHECK_INPUT(p_offset)
  ...
  // Gradients always in FP32 for sampling
  auto dtype = value.dtype();
  if (dtype == at::kHalf) {
    dtype = at::kFloat;
  }

  // Allocate memory for the gradients w.r.t. the input and offset tensors
  ...
  for (int n = 0; n < batch / im2col_step_; ++n) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, value.scalar_type(),
        "deform2d::backward_cuda", ([&] {
          col2im_cuda(at::cuda::getCurrentCUDAStream(),
                      value.data_ptr<scalar_t>() +
                          n * im2col_step_ * per_value_size,
                      p_offset.data_ptr<scalar_t>() +
                          n * im2col_step_ * per_offset_size,
  ...
  // Return the gradients w.r.t. the input and offset tensors
...
TORCH_LIBRARY_IMPL(deformops, CUDA, m) {
  m.impl("deform2d_forward", &deform2d::forward_cuda);
  m.impl("deform2d_backward", &deform2d::backward_cuda);
}
